...
file once and you understand every background job in Inferio.
...
class LlmStreamWorker(QtCore.QThread):
    ...
    chunk_received = QtCore.Signal(str)   # a few characters of new text
    finished_ok = QtCore.Signal(str)      # the complete response text
    ...
    def run(self):  # executes on the WORKER thread
        from inferio.llm.client import GenerationCancelled
        try:
...
class ServerStartWorker(QtCore.QThread):
    ...
    def run(self):
        from inferio.llm.server import MANAGER, LOG_PATH
        try:
