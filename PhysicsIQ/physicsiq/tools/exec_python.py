...
def _build_globals():
    ...
    import Part
    from physicsiq.tools import registry
    ...
    g = {
        ...
        "Part": Part,
        "math": math,
        "piq_tools": registry.make_namespace(),
    }
    try:
        import Draft
        g["Draft"] = Draft
...
def run_freecad_python(code: str, transaction_name="PhysicsIQ AI edit") -> ExecResult:
    import FreeCAD as App
    import FreeCADGui as Gui
    ...
    if doc is None:
        doc = App.newDocument("PhysicsIQ")

    out = io.StringIO()
    doc.openTransaction(transaction_name)  # <- the undo checkpoint
    try:
        with contextlib.redirect_stdout(out):
            exec(compile(code, "<physicsiq-generated>", "exec"), _build_globals())
